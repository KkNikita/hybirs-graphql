...
public class DefaultOutboundRequestResponseInterceptor implements ClientHttpRequestInterceptor
{
	...
	protected ClientHttpResponse handleError(final OutboundRequestModel outboundRequestModel, final byte[] payload,
			...
			throws IOException
	{
		final var response = new de.hybris.platform.outboundservices.monitoring.WrappedClientHttpResponse(clientHttpResponse,
				getOutboundServicesConfiguration().getMaximumResponsePayloadSize());
		outboundRequestModel.setStatus(IntegrationRequestStatus.ERROR);
		outboundRequestModel.setError(
				response.isResponseMaxSizeExceeded() ? EXCEEDED_RESPONSE_PAYLOAD_MAX_SIZE_ERROR
	...
	/**
	 * @deprecated Please use {@link de.hybris.platform.outboundservices.monitoring.WrappedClientHttpResponse} instead.
	 */
	@Deprecated(since = "21.05", forRemoval = true)
	public static class WrappedClientHttpResponse extends de.hybris.platform.outboundservices.monitoring.WrappedClientHttpResponse
	{

		public WrappedClientHttpResponse(final ClientHttpResponse clientHttpResponse, final int sizeLimit)
		{
			super(clientHttpResponse, sizeLimit);
		}
	}
}

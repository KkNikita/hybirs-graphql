...
class ODataReadEntitySetIntegrationTest extends ServicelayerSpockSpecification {
    private static final String TEST_NAME = 'ODataReadEntitySet'
    private static final String IO = "${TEST_NAME}_GetIntegrationTestIO"
    ...
    def "get all Packages when no Packages EntitySet has been defined"() {
        ...
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        responseBody.getString('$.error.message.value') == "Could not find an entity set or function import for 'Packages'."
    }
    ...
    def "get all Products when no Products exist"() throws ImpExException {
        ...
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        responseBody.getCollectionOfObjects(ALL_RESULTS_PATH).isEmpty()
    }
    ...
    def "get all products with no url query parameters returns default page size of products"() {
        ...
        oDataResponse.status == HttpStatusCodes.OK
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        responseBody.getCollectionOfObjects(ALL_RESULTS_PATH).size() == DEFAULT_PAGE_SIZE
        !responseBody.exists(COUNT_PATH)
        responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIPTOKEN + "=" + DEFAULT_PAGE_SIZE)
    }
    ...
    def 'get all products with $top returns the top number of products and next link'() {
        ...
        oDataResponse.status == HttpStatusCodes.OK
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, product1Filter).isEmpty()
        responseBody.getCollection(ALL_RESULTS_PATH).size() == requestedTop
        !responseBody.exists(COUNT_PATH)
        responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIPTOKEN + "=" + requestedTop)
    }
    ...
    def 'get all products with $top exceeding max page size returns max page size of items'() {
        ...
        oDataResponse.status == HttpStatusCodes.OK
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        responseBody.getCollection(ALL_RESULTS_PATH).size() == MAX_PAGE_SIZE
        !responseBody.exists(COUNT_PATH)
        responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIPTOKEN + "=" + MAX_PAGE_SIZE)
    }
    ...
    def 'get all products with $inlinecount=none and $top=2 returns the first 2 products and nextlink'() {
        ...
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, product1Filter).isEmpty()
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, product2Filter).isEmpty()
        responseBody.getCollection(ALL_RESULTS_PATH).size() == requestedTop
        !responseBody.exists(COUNT_PATH)
        responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIPTOKEN + "=" + requestedTop)
    }
    ...
    def 'get all products with $inlinecount=allpages and $top=1 returns the first product, count and nextlink'() {
        ...
        oDataResponse.status == HttpStatusCodes.OK
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, Filter.filter(Criteria.where(CODE).eq(products.get(0)))).isEmpty()
        responseBody.getCollection(ALL_RESULTS_PATH).size() == requestedTop
        responseBody.getString(COUNT_PATH) == PRODUCTS_ENTITY_SET_SIZE
        responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIPTOKEN + "=" + requestedTop)
    }
    ...
    def 'get all products with $top=2 and $skip=1 returns the 2nd through 3rd product, count and nextlink'() {
        ...
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, product2Filter).isEmpty()
        !responseBody.exists(COUNT_PATH)
        !responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIP + "=")
        responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIPTOKEN + "=" + (requestedTop + requestedSkip))
    }
    ...
    @Test
    def 'get all products with $top=1 and $skiptoken=2 returns the 3rd product and nextlink'() {
        given:
        givenProductsExist()
        final int requestedTop = 1
        final int requestedSkip = 2
        final ODataRequestBuilder oDataRequest = requestBuilder(PRODUCTS_ENTITYSET)
                .withParameter(SKIPTOKEN, requestedSkip)
                .withParameter(TOP, requestedTop)
        ...
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, product1Filter).isEmpty()
        !responseBody.exists(COUNT_PATH)
        StringUtils.countMatches(responseBody.getString(NEXT_PAGE_LINK_PATH), SKIPTOKEN + "=") == 1
        responseBody.getString(NEXT_PAGE_LINK_PATH).contains(SKIPTOKEN + "=" + (requestedTop + requestedSkip))
    }
    ...
    def 'get all products with $inlinecount=allpages, $top=2 and $skip=3 returns last page with 1 product, count and NO nextlink'() {
        ...
        oDataResponse.status == HttpStatusCodes.OK
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        responseBody.getCollection(ALL_RESULTS_PATH).size() == 1
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, product1Filter).isEmpty()
        !responseBody.exists(NEXT_PAGE_LINK_PATH)
        responseBody.getString(COUNT_PATH) == "4"
    }

    @Test
    def 'get all products with $inlinecount=allpages, $top=2 and $skiptoken=3 returns last page with 1 product, count and NO nextlink'() {
        given:
        givenProductsExist()
        final Integer requestedSkip = 3
        final ODataRequestBuilder oDataRequest = requestBuilder(PRODUCTS_ENTITYSET)
    ...
    def 'get products with $skip=# exceeding the number of existing products'() {
        ...
        oDataResponse.status == HttpStatusCodes.OK
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        responseBody.getCollection(ALL_RESULTS_PATH).isEmpty()
        responseBody.getString(COUNT_PATH) == "1"
        !responseBody.exists(NEXT_PAGE_LINK_PATH)
    }
    ...
    def 'get products $count'() {
        ...
        then:
        oDataResponse.status == HttpStatusCodes.OK
        String.valueOf(oDataResponse.getEntity()) == PRODUCTS_ENTITY_SET_SIZE
    }
    ...
    def "get a specific products collection attribute"() {
        ...
        oDataResponse.status == HttpStatusCodes.OK
        def responseBody = JsonObject.createFrom oDataResponse.entityAsStream
        responseBody.getCollection(ALL_RESULTS_PATH).size() == 2
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, category1Filter).isEmpty()
        !responseBody.getFilteredCollection(SOME_RESULTS_PATH, category2Filter).isEmpty()
    }

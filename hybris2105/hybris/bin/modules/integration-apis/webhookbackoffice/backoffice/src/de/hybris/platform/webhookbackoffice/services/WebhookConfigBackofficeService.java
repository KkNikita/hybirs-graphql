...
public class WebhookConfigBackofficeService
{
	...
	private static final String FIND_WEBHOOKCONFIG =
			" SELECT DISTINCT {" + ItemModel.PK + "}" + " FROM {" + WebhookConfigurationModel._TYPECODE + " as wbh}" + " WHERE {wbh:"
					+ WebhookConfigurationModel.EVENTTYPE + "}=?" + EVENT_CLASS + " AND " + "{wbh:"
					+ WebhookConfigurationModel.INTEGRATIONOBJECT + "}=?" + IO + " AND " + "{wbh:"
					+ WebhookConfigurationModel.DESTINATION + "}=?" + DESTINATION;
	private static final String FIND_ACTIVE_GROOVY_SCRIPTS =
			" SELECT DISTINCT {" + ItemModel.PK + "}" + " FROM {" + ScriptModel._TYPECODE + " as s JOIN " + ScriptType._TYPECODE
					+ " as st ON {s:" + ScriptModel.SCRIPTTYPE + "}={st:pk}}"
	                                                               ...
	                                                               @NotNull final String eventType)
	{
		...
		final Map<String, Object> params = Map.of(EVENT_CLASS, eventType, IO, integrationObject.getPk(), DESTINATION,
	...
	public WebhookConfigurationModel createWebhookConfiguration(@NotNull final IntegrationObjectModel integrationObject,
	                                                            @NotNull final ConsumedDestinationModel consumedDestination,
	                                                            final String filterLocation, final String eventType)
	{
		Preconditions.checkArgument(integrationObject != null, "integrationObject cannot be null");
		Preconditions.checkArgument(consumedDestination != null, "consumedDestination cannot be null");

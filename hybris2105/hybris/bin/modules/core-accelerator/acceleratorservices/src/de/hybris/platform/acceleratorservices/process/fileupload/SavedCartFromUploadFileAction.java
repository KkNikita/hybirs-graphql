...
public class SavedCartFromUploadFileAction extends AbstractSimpleDecisionAction<SavedCartFileUploadProcessModel>
{
	...
	protected ImpersonationContext createImpersonationContext(final SavedCartFileUploadProcessModel cartFileUploadProcessModel)
	{
		...
		if(cartFileUploadProcessModel.getSite() instanceof CMSSiteModel){
			CMSSiteModel siteModel = (CMSSiteModel) cartFileUploadProcessModel.getSite();
			CatalogVersionModel defaulActiveCatalogVersion = siteModel.getDefaultCatalog() == null ? null : siteModel.getDefaultCatalog().getActiveCatalogVersion();
			impersonationContext.setCatalogVersions(Arrays.asList(defaulActiveCatalogVersion));
		}

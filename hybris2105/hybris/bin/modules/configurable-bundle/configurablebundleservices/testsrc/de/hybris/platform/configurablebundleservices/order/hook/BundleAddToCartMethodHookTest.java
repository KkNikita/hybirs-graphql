...
public class BundleAddToCartMethodHookTest
{
	...
	public void shouldCheckTheComponentBelongToTheRightTemplate() throws CommerceCartModificationException
	{
		...
		group.setGroupNumber(Integer.valueOf(1));
		group.setGroupType(GroupType.CONFIGURABLEBUNDLE);
		when(bundleTemplateService.getBundleEntryGroup(cart, parameter.getEntryGroupNumbers())).thenReturn(group);
		when(bundleTemplateService.getRootBundleTemplate(component1)).thenReturn(new BundleTemplateModel());
		when(bundleTemplateService.getRootBundleTemplate(component2)).thenReturn(component2);
		when(bundleTemplateService.getBundleTemplateName(component1)).thenReturn(component1.getId());
		when(bundleTemplateService.getBundleTemplateName(component2)).thenReturn(component2.getId());
	...
	public void shouldNotCheckRequiredComponentNullEntryGroup() throws CommerceCartModificationException
	{
		...
		when(bundleTemplateService.getBundleEntryGroup(parameter.getCart(), parameter.getEntryGroupNumbers())).thenReturn(null);
		when(bundleTemplateService.getBundleTemplateForCode("A")).thenReturn(component1);
	...
	public void shouldCreateBundleTreeInCart() throws CommerceCartModificationException
	{
		...
		when(bundleTemplateService.getRootBundleTemplate(component1)).thenReturn(component1);
		when(Integer.valueOf(entryGroupService.findMaxGroupNumber(any()))).thenReturn(Integer.valueOf(0));
		...
		when(bundleTemplateService.getBundleEntryGroup(any(), any())).thenReturn(group);

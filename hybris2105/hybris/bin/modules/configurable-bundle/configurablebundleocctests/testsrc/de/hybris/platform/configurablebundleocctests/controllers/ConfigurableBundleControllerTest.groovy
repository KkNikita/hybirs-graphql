...
class ConfigurableBundleControllerTest extends AbstractCartTest{
    ...
    def "Validation should fail for missing required attribute #missingAttribute"() {
        ...
        then: "A MethodArgumentNotValidException Error is returned"
        with(response) {
            status == SC_BAD_REQUEST
            def dataMap = data.errors.collectEntries{[it.type, it]}

            dataMap.containsKey("MethodArgumentNotValidError")
            dataMap["MethodArgumentNotValidError"].message.contains(missingAttribute)
        }
    ...
    def "should fail for invalid attribute #invalidAttribute"() {
        ...
        with(response) {
            ...
            dataMap.containsKey(errorType)
            dataMap[errorType].message.contains(errorMessage)
        }
        ...
        "quantity"          | "MethodArgumentNotValidError" | "quantity"                        | "{\"templateId\": \"PhotoOTGCameraComponent\", \"productCode\": \""+PRODUCT_EOS_40D_BODY_VALUE+"\", \"quantity\": \"-1\"}"
    ...
    @IgnoreIf({!BundleTestSetupUtils.enableAutoPick()})
    def "Should start a bundle with valid auto-pick product in cart"() {
        given: "anonymous user with cart"
        def user = ['id': 'anonymous']
        def cart = createAnonymousCart(restClient, JSON)
        ...
        then: "The product(805693) in auto-pick component will be added to the cart automatically"
        with(responseCart) {
            status == SC_OK
            data.entryGroups.size() == 1
            data.entryGroups[0].entryGroups[0].entries[0].product.code == "816802"
            data.entryGroups[0].entryGroups[0].entries[0].quantity == 1
            data.entryGroups[0].entryGroups[0].entries[0].entryNumber == 0

            data.entryGroups[0].entryGroups[1].entries[0].product.code == "805693"
            data.entryGroups[0].entryGroups[1].entries[0].quantity == 1
            data.entryGroups[0].entryGroups[1].entries[0].entryNumber == 1
        }
    }
    ...
    @IgnoreIf({!BundleTestSetupUtils.enableAutoPick()})
    def "Should not add starter product to cart twice when auto pick component is also starter"() {
        given: "anonymous user with cart"
        def user = ['id': 'anonymous']
        def cart = createAnonymousCart(restClient, JSON)
        ...
        def response = restClient.post(
                path: path,
                body: [
                        "templateId": "PhotoAutoPickStarterCameraNoDuplicationComponent",
                        "productCode": "816802",
                        "quantity": 1
                ],
                contentType: JSON,
        ...
        then: "The product(816802) in auto-pick component won't be added twice to the cart"
        with(responseCart) {
            status == SC_OK
            data.entryGroups.size() == 1
            data.entryGroups[0].entryGroups[0].entries[0].product.code == "816802"
    ...
    @IgnoreIf({!BundleTestSetupUtils.enableAutoPick()})
    def "Should not filter out auto-pick product from second component if the same product is starter"() {
        given: "anonymous user with cart"
        def user = ['id': 'anonymous']
        def cart = createAnonymousCart(restClient, JSON)
        ...
        def response = restClient.post(
                path: path,
                body: [
                        "templateId": "PhotoAutoPickStarterCameraComponent",
                        "productCode": "816802",
                        "quantity": 1
                ],
                contentType: JSON,
        ...
        then: "The products(805693, 816802) in auto-pick component will be added to the cart automatically without filter out"
        with(responseCart) {
            status == SC_OK
            data.entryGroups.size() == 1
            data.entryGroups[0].entryGroups[0].entries[0].product.code == "816802"
            ...
            data.entryGroups[0].entryGroups[1].entries[1].product.code == "816802"
            data.entryGroups[0].entryGroups[1].entries[1].quantity == 1
            data.entryGroups[0].entryGroups[1].entries[1].entryNumber == 2
        }
    }
}

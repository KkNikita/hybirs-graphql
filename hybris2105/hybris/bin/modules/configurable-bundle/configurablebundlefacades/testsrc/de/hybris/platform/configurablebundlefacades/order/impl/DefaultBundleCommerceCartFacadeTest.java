...
public class DefaultBundleCommerceCartFacadeTest
{
	...
	public void shouldCheckBundleIntegrity() throws CommerceCartModificationException
	{
		...
		given(entryGroupService.getGroup(cart, Integer.valueOf(2)))
				.willThrow(new IllegalArgumentException("No group with number #2 in the order with code A"));
	...
	public void shouldDeleteBundleByEntryGroup() throws CommerceCartModificationException
	{
		...
		final ArgumentCaptor<CommerceCartParameter> captor = ArgumentCaptor.forClass(CommerceCartParameter.class);
		given(commerceCartService.updateQuantityForCartEntry(captor.capture())).willReturn(null);
	...
	public void testStartBundleWithAutoPick() throws CommerceCartModificationException
	{
		...
		when(bundleTemplateService.getBundleTemplateForCode("bundleTemplate")).thenReturn(result);

		final ProductModel product = new ProductModel();
		String productCode1 = "code1";
		product.setCode(productCode1);
		when(productService.getProductForCode(productCode1)).thenReturn(product);
		...
		when(entryGroupService.getRoot(any(), any())).thenReturn(root);
		when(bundleTemplateService.getBundleTemplateForCode("child")).thenReturn(autoPick);
		when(bundleTemplateService.getBundleEntryGroup(any(), any())).thenReturn(root);
		when(bundleTemplateService.isAutoPickComponent(autoPick)).thenReturn(true);
	...
	public void testStartBundleWithAutoPickForStarter() throws CommerceCartModificationException
	{
		...
		when(bundleTemplateService.getBundleEntryGroup(any(), any())).thenReturn(root);
		when(bundleTemplateService.isAutoPickComponent(autoPick)).thenReturn(true);
		when(bundleTemplateService.isAutoPickComponent(result)).thenReturn(true);

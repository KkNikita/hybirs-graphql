...
public class CategoryPageRedirectSupplierTest
{
	...
	{
		when(request.getRequestURI()).thenReturn("apparel-uk/en/cms/components");
		assertFalse(supplier.shouldRedirect(request, previewData));
	}

	@Test
	public void shouldNotRedirectWhenRequestCMSPageWithPageId()
	{
		when(request.getRequestURI()).thenReturn("apparel-uk/en/cms/pages/12345");
		assertFalse(supplier.shouldRedirect(request, previewData));
	}
	...
	{
		when(request.getRequestURI()).thenReturn("apparel-uk/en/cms/pages");
		when(request.getParameter(PAGE_TYPE)).thenReturn("PrductPage");
		when(typeCodeNegatedPredicate.test("PrductPage")).thenReturn(true);
		when(typeCodePredicate.negate()).thenReturn(typeCodeNegatedPredicate);
	...
	public void shouldNotRedirectWhenRequestCMSPageWithPageTypeAndId()
	{
		...
		when(typeCodeNegatedPredicate.test("PrductPage")).thenReturn(true);
		when(typeCodePredicate.negate()).thenReturn(typeCodeNegatedPredicate);
		when(request.getParameter(PAGE_LABEL_ID)).thenReturn("12345");

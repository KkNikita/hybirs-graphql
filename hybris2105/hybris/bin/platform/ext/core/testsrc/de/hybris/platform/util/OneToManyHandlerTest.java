...
public class OneToManyHandlerTest extends HybrisJUnit4Test
{
	...
	private static final int PREVIOUS_POSITION = 5;
	...
	{
		product2FeatureRelation.switchToValue("invalidOrEmpty");
		final OneToManyHandler<?> spiedHandler = Mockito.spy(handler);
		final Item.ItemAttributeMap attributes = new Item.ItemAttributeMap();
	...
	public void shouldUsePositionFromAttributesWhenPropertyIsInvalidAndValueIsProvided()
	{
		...
		final Product product = new Product();
		attributes.put("product", product);
		attributes.put(POSITION, PREVIOUS_POSITION);
		...
		//then
		Mockito.verify(spiedHandler, Mockito.never()).getNextOrderNumber(ctx, product);
		assertThat(attributes.get(POSITION)).isEqualTo(PREVIOUS_POSITION);
	}
}
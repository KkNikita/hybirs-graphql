...
var attrDatasource1 = "#dataSource1";
var attrDatasource2 = "#dataSource2";
...
$(function () {
    ...
    $(attrDatasource1).change(function () {
        $(attrDatasource2).val($(attrDatasource1).val());
    });
    $(attrDatasource2).change(function () {
        $(attrDatasource1).val($(attrDatasource2).val());
    });
    //========================================

    //Make sure that data from read-only replicas are reached on rollback mode.
    function disableCommitWhenReadOnly() {
        if ($(attrDatasource1).val().indexOf("readonly") > -1 || $(attrDatasource2).val().indexOf("readonly") > -1) {
            if ($("#commitCheckbox1").is(":checked")) {
                $("#commitCheckbox1").click();
            }
            $("#commitCheckbox1").attr("disabled", true);
            $("#commitCheckbox2").attr("disabled", true);
        } else {
            $("#commitCheckbox1").attr("disabled", false);
            $("#commitCheckbox2").attr("disabled", false);
        }
    }

    //Initially disable commit button for read-only replicas.
    disableCommitWhenReadOnly();
    $(attrDatasource1).change(function () {
        disableCommitWhenReadOnly();
    });
    $(attrDatasource2).change(function () {
    ...
    function encryptWriteCookie(historyItem, name) {
        var cookieObject = {
            ...
            additionalDescription: historyItem.additionalDescription,
            isFs: historyItem.isFs
        };
        ...
        $('#flexQueryForm').validate({
            ...
            submitHandler: function (form) {
                ...
                var dataObject = {
                    ...
                    user: $("#user1").val(),
                    locale: $("#locale1").val(),
                    dataSource: $(attrDatasource1).val(),
